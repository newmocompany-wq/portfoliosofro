...
function EduModal({ initial, onClose, onSaved }) {
  const [form, setForm] = useState({ ...EMPTY, ...(initial ?? {}), school: initial?.school ?? initial?.institution ?? "" });
  const [saving, setSaving] = useState(false);
  const set = (k, v) => setForm(f => ({ ...f, [k]: v }));
  const submit = async (e) => {
    e.preventDefault(); setSaving(true);
    try { initial?.id ? await api.education.update(initial.id, form) : await api.education.create(form); onSaved(); }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div><h1 className="text-3xl font-bold font-display">Education</h1><p className="text-sm text-muted-foreground mt-1">Degrees and academic credentials</p></div>
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          <thead><tr className="border-b border-border">
            ...
            <th className="px-4 py-3 text-left text-[11px] font-mono uppercase tracking-widest text-muted-foreground">Degree</th>
            <th className="px-4 py-3 text-left text-[11px] font-mono uppercase tracking-widest text-muted-foreground">School</th>
            <th className="px-4 py-3 text-left text-[11px] font-mono uppercase tracking-widest text-muted-foreground">Year</th>
          ...
          <tbody>
            {paginated.map(item => (
              <tr key={item.id} className="border-b border-border/60 last:border-0 hover:bg-muted/30 transition-colors">
                ...
                <td className="px-4 py-3 text-muted-foreground">{item.school ?? item.institution}</td>
