  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <button onClick={() => nav("/admin/researches/new")}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border">
              ...
              <th className="px-4 py-3 text-left text-[11px] font-mono uppercase tracking-widest text-muted-foreground">Status</th>
          ...
          <tbody>
            {paginated.map(item => (
              <tr key={item.id} className="border-b border-border/60 last:border-0 hover:bg-muted/30 transition-colors">
                ...
                <td className="px-4 py-3">
                  <span className={`text-[11px] font-mono px-2 py-0.5 rounded capitalize ${STATUS_CHIP[item.status] ?? STATUS_CHIP.draft}`}>{item.status}</span>
                </td>
                <td className="px-4 py-3">
                  <div className="flex items-center justify-end gap-1">
                    <button onClick={() => nav(`/admin/researches/${item.id}/edit`)}
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={5} className="text-center py-12 text-muted-foreground text-sm">No research papers found</td></tr>
            )}
          </tbody>
        </table>
