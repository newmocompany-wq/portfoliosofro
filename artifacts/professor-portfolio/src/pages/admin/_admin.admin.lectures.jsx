...
function LectureModal({ initial, courses, onClose, onSaved }) {
  ...
  const submit = async (e) => {
    ...
    try { initial?.id ? await api.courses.update(initial.id, form) : await api.courses.create(form); onSaved(); }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div><h1 className="text-3xl font-bold font-display">Lectures</h1><p className="text-sm text-muted-foreground mt-1">All lecture materials across courses</p></div>
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          <thead><tr className="border-b border-border">
            ...
            <th className="px-4 py-3 text-left text-[11px] font-mono uppercase tracking-widest text-muted-foreground">Course</th>
