      ...
      <div className="bg-card border border-border rounded-xl w-full max-w-lg flex flex-col shadow-2xl">
        ...
        <div className="px-6 py-5 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div><p className="text-[11px] font-mono uppercase tracking-widest text-muted-foreground mb-1">From</p><p className="text-sm font-medium">{msg.name}</p></div>
            <div><p className="text-[11px] font-mono uppercase tracking-widest text-muted-foreground mb-1">Date</p><p className="text-sm text-muted-foreground">{msg.date}</p></div>
          </div>
          <div><p className="text-[11px] font-mono uppercase tracking-widest text-muted-foreground mb-1">Email</p><a href={`mailto:${msg.email}`} className="text-sm text-electric hover:underline">{msg.email}</a></div>
          {msg.subject && <div><p className="text-[11px] font-mono uppercase tracking-widest text-muted-foreground mb-1">Subject</p><p className="text-sm font-medium">{msg.subject}</p></div>}
          <div><p className="text-[11px] font-mono uppercase tracking-widest text-muted-foreground mb-1">Message</p><p className="text-sm text-muted-foreground leading-relaxed">{msg.body ?? msg.message}</p></div>
        </div>
        <div className="flex justify-end px-6 py-4 border-t border-border">
          <a href={`mailto:${msg.email}`} className="px-4 py-2 rounded-lg bg-electric text-electric-foreground text-sm font-medium hover:opacity-90 transition">Reply by Email</a>
        </div>
      </div>
    </div>
  );
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">{unread > 0 ? <><span className="text-electric font-medium">{unread} unread</span> · {items.length} total</> : `${items.length} messages`}</p>
        </div>
      </div>
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          <thead><tr className="border-b border-border">
            ...
            <th className="px-4 py-3 text-left text-[11px] font-mono uppercase tracking-widest text-muted-foreground">Name</th>
            <th className="px-4 py-3 text-left text-[11px] font-mono uppercase tracking-widest text-muted-foreground">Subject</th>
            ...
            {paginated.map(msg => (
              <tr key={msg.id} onClick={() => open(msg)} className={`border-b border-border/60 last:border-0 cursor-pointer hover:bg-muted/30 transition-colors ${!msg.read ? "font-semibold" : ""}`}>
                ...
                <td className="px-4 py-3"><div className="flex items-center justify-end gap-1">
                  <button onClick={(e) => del(msg.id, e)} className="grid size-8 place-items-center rounded-md hover:bg-destructive/10 text-muted-foreground hover:text-destructive transition"><Trash2 className="size-4" /></button>
                </div></td>
              </tr>
            ))}
