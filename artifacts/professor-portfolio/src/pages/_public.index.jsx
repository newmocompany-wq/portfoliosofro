...
import { ArrowRight, BookOpen, GraduationCap, Award, Briefcase, Quote, Mail, FileText, Sparkles } from "lucide-react";
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      if (!del) {
        ...
        setText(cur.slice(0, text.length - 1));
        if (text.length - 1 === 0) { setDel(false); setI(x => (x + 1) % list.length); }
      }
    }, del ? 30 : 60);
      ...
      <div className="grid gap-10 md:grid-cols-2 items-center">
        <div className="relative">
          ...
          {professor.vision && (
            <div className="absolute -bottom-6 -right-6 glass rounded-xl p-4 max-w-[260px]">
              ...
              <p className="text-sm italic text-muted-foreground">"{professor.vision.slice(0, 120)}{professor.vision.length > 120 ? "…" : ""}"</p>
            </div>
          )}
        </div>
        <div className="space-y-5">
          <SectionHeader
            ...
            title={<>A career spent at the <span className="text-gradient-electric">edge of wireless</span></>}
            subtitle={null}
            align="left"
          />
          ...
          {professor.interests?.length > 0 && (
            <div className="grid grid-cols-2 gap-3">
              {professor.interests.slice(0, 4).map(interest => (
                <div key={interest} className="flex items-center gap-2 rounded-md border border-border bg-card px-3 py-2 text-sm">
                  <span className="size-1.5 rounded-full bg-electric" /> {interest}
                </div>
              ))}
            </div>
      ...
      <section className="container-academic py-16">
        ...
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {courses.slice(0, 3).map(c => (
            <CoverCard key={c.id} to={`/courses/${c.id}`} cover={c.cover} eyebrow={`${c.lectures?.length || 0} Lectures`} title={c.title} meta={c.description} />
          ))}
        </div>
      </section>
    )}
