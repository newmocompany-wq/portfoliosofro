  ...
  return <>
      ...
      <section className="container-academic py-10">
        ...
        {isLoading ? <Spinner /> : !data?.data.length ? <Empty /> : <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            {data.data.map(r => <CoverCard key={r.id} to={`/researches/${r.id}`} cover={r.cover} eyebrow={String(r.year)} title={r.title} meta={r.abstract} footer={`${r.authors.join(", ")} • ${r.journal}`} />)}
          </div>}
