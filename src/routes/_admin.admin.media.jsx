...
function MediaPage() {
  ...
  const remove = useMutation({
    mutationFn: id => api.media.remove(id),
    onSuccess: () => {
  ...
  return <div className="space-y-6">
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        ...
        {pending && <div className="mt-3 flex flex-wrap items-center gap-3">
            <input value={pendingName} onChange={e => setPendingName(e.target.value)} placeholder="File name (optional)" className="flex-1 min-w-[200px] rounded-md border border-input bg-background px-3 py-2 text-sm" />
            <button disabled={upload.isPending} onClick={() => upload.mutate()} className="rounded-md bg-electric px-5 py-2 text-sm font-medium text-electric-foreground hover:opacity-90 disabled:opacity-60">
      ...
      {isLoading ? <Spinner /> : !data?.data.length ? <Empty /> : <div className="grid gap-4 grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">
          {data.data.map(m => {
        const Icon = ICON[m.type] ?? FileIcon;
        return <div key={m.id} className="group relative rounded-xl border border-border bg-card overflow-hidden">
