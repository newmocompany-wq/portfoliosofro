...
function BlogPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ["pub-blog", search, page], queryFn: () => api.blogs.list({ search, page, pageSize: 9 }) });
  return (
    <>
      ...
      <section className="container-academic py-10">
        ...
        {isLoading ? <Spinner /> : !data?.data.length ? <Empty /> : (
          <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            {data.data.map((b) => (
              <CoverCard key={b.id} to={`/blog/${b.id}`} cover={b.cover} eyebrow={new Date(b.date).toLocaleDateString()} title={b.title} meta={b.excerpt} />
            ))}
          </div>
        )}
