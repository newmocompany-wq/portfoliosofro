...
function AchievementsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.achievements.list({ search, page, pageSize: 9 }),
  });
    ...
    <>
      ...
      <section className="container-academic py-10">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <SearchInput value={search} onChange={(v) => { setSearch(v); setPage(1); }} placeholder="Search achievements…" />
        ...
        {isLoading ? <Spinner /> : !data?.data.length ? <Empty /> : (
          <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            {data.data.map((a) => (
              <CoverCard key={a.id} to={`/achievements/${a.id}`} cover={a.cover} eyebrow={a.category} title={a.title}
                meta={a.description} footer={new Date(a.date).toLocaleDateString()} />
            ))}
          </div>
        )}
